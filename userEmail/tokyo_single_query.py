...
CONFIG_PATH = "tokyo_query_config.json"
# 全局已通知集合
notified_set = set()
                    ...
                    if slot_img:
                        ...
                        if slot_alt == "空き" and notify_key not in notified_set:
                            ...
                            try:
                                ...
                                resp = requests.post(
                                    "https://gander-stiffen-gently.ngrok-free.dev/api/send-venue-email",  # 替换为你的API地址
                                    json=payload,
                                    timeout=10
                                )
                                if resp.ok:
            ...
            if other_month_dates and today.day > 18:
                ...
                if next_month_btn:
                    ...
                    for query_date in other_month_dates:
                        ...
                        for slot in query_time_slots:
                            slot_td_id = f"{query_date.replace('-', '')}_{slot_map.get(slot, '10')}"
                            slot_td_elem = page.query_selector(f'td[id="{slot_td_id}"]')
                            if not slot_td_elem:
                                result[slot] = "无数据"
                                continue
                            slot_img = slot_td_elem.query_selector('img.calendar-status')
                            if slot_img:
                                slot_alt = slot_img.get_attribute('alt')
                                result[slot] = slot_alt
                                notify_key = f"{building_name}_{query_date}_{slot}"
                                if slot_alt == "空き" and notify_key not in notified_set:
                                    try:
                                        payload = {
                                            ...
                                            "timeRange": slot,
                                            "date": query_date,
                                            "email": "ganjiabubuzhang@gmail.com"
                                        }
                                        resp = requests.post(
                                            "https://gander-stiffen-gently.ngrok-free.dev/api/send-venue-email",
                                            json=payload,
                                            timeout=10
                                        )
                                        if resp.ok:
