...
CONFIG_PATH = "userEmail2_config.json"
# 全局已通知集合
notified_set = set()
...
def query_tokyo_reservation(url, user_id, password, building_id, building_name, query_dates, query_time_slots, email):
    ...
    try:
        with sync_playwright() as p:
            ...
            # 若为场馆 1130，覆盖 query_dates 为当月和下月所有日期
            if building_id == "1130":
                today_dt = datetime.date.today()
                y, m = today_dt.year, today_dt.month
                # 当月所有日期
                last_day_curr = calendar.monthrange(y, m)[1]
                curr_dates = [f"{y}-{m:02d}-{d:02d}" for d in range(1, last_day_curr + 1)]
                # 下月所有日期
                nm = m + 1
                ny = y
                if nm == 13:
                ...
                next_dates = [f"{ny}-{nm:02d}-{d:02d}" for d in range(1, last_day_next + 1)]
                # 过滤：当月仅保留今日及之后的日期
                try:
                    curr_dates_filtered = [
                        d for d in curr_dates
                        if datetime.datetime.strptime(d, "%Y-%m-%d").date() >= today_dt
                    ]
                except Exception:
                    # 若解析异常则回退为原始当月日期
                    curr_dates_filtered = curr_dates
                dates_to_query = curr_dates_filtered
                other_month_dates = next_dates
                print(f"building 1130: 使用当月（今日及之后）共 {len(dates_to_query)} 天，次月共 {len(other_month_dates)} 天")
            else:
                ...
                if len(query_dates) == 1:
                    ...
                    dates_to_query = [d for d in query_dates if same_month(d, first_date)]
                    other_month_dates = [d for d in query_dates if not same_month(d, first_date)]
            ...
            for query_date in dates_to_query:
                ...
                # 针对 building_id == "1130" 的特殊规则：
                # 如果该日期在传入的 query_dates 中则直接视为 True（按要求），
                # 否则即为False
                try:
                    if query_date in query_dates:
                        is_weekend = True
                    else:
                        is_weekend = False
                except Exception:
                    # 无法解析日期时回退到默认行为（保守处理为 True）
                    is_weekend = True

                if building_id == "1130" and not is_weekend:
                    slots_to_check = ["19:00-21:00"]
                else:
                    slots_to_check = query_time_slots                
